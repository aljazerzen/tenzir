...
auto proxy_from_env() -> Option<std::string> {
  for (auto const* name : {"HTTPS_PROXY", "https_proxy", "HTTP_PROXY",
                           "http_proxy"}) {
...
auto parse_proxy_url(std::string const& url, proxy_settings& out)
  ...
  if (scheme != "http" and scheme != "https") {
    return caf::make_error(
      ec::invalid_configuration,
      fmt::format("tenzir.http-proxy has unsupported scheme `{}`; expected "
                  "http or https",
                  scheme));
  }
  ...
  try {
    ...
    return caf::make_error(
      ec::invalid_configuration,
      fmt::format("tenzir.http-proxy has invalid port `{}`: {}", port_text,
                  e.what()));
  }
...
auto mirror_to_environment(proxy_settings const& ps) -> caf::error {
  ...
  if (ps.no_proxy) {
    for (auto const* name : {"NO_PROXY", "no_proxy"}) {
      if (auto err = set(name, *ps.no_proxy)) {
        return err;
      }
    }
  }
...
auto is_loopback(std::string_view host) -> bool {
  ...
  // Match any IPv4 loopback (127.0.0.0/8). The bracket-stripped IPv6
  // loopback was already handled above.
